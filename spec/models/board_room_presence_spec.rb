...
RSpec.describe BoardRoomPresence do
  subject(:board_room_presence) { build_stubbed(:board_room_presence) }
  ...
  describe '#valid?' do
    ...
    context 'when without start time' do
      subject(:board_room_presence) do
        build_stubbed(:board_room_presence, start_time: nil)
      end
    ...
    context 'when without end time' do
      subject(:board_room_presence) do
        build_stubbed(:board_room_presence, end_time: nil)
      end
    ...
    context 'when end time before start time' do
      subject(:board_room_presence) do
        build_stubbed(:board_room_presence,
    ...
    context 'when without status' do
      subject(:board_room_presence) { build_stubbed(:board_room_presence, status: nil) }

      it { expect(board_room_presence).not_to be_valid }
    end

    context 'when without a user' do
      subject(:board_room_presence) { build_stubbed(:board_room_presence, user: nil) }
  ...
  describe '#current_and_future' do
    ...
    context 'when starting in the future' do
      before { create(:board_room_presence, start_time: 1.second.from_now) }
