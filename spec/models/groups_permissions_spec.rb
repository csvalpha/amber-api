...
RSpec.describe GroupsPermissions, type: :model do
  subject(:groups_permissions) { FactoryBot.build_stubbed(:groups_permissions) }

  describe '#valid?' do
    it { expect(groups_permissions.valid?).to be true }

    context 'when without a group' do
      subject(:groups_permissions) { FactoryBot.build_stubbed(:groups_permissions, group: nil) }
    ...
    context 'when without a permission' do
      subject(:groups_permissions) do
        FactoryBot.build_stubbed(:groups_permissions, permission: nil)
      end
