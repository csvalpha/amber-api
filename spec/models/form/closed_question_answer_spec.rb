...
RSpec.describe Form::ClosedQuestionAnswer do
  subject(:closed_question_answer) { build(:closed_question_answer) }
  ...
  describe '#valid' do
    ...
    context 'when without a response' do
      subject(:closed_question_answer) do
        build(:closed_question_answer, response: nil)
      end
    ...
    context 'when without an option' do
      subject(:closed_question_answer) do
        build(:closed_question_answer, option: nil)
      end
    ...
    context 'when question does not belong to form' do
      let(:closed_question) { create(:closed_question) }
      let(:another_form) { create(:form) }

      subject(:another_closed_question_answer) do
        build(:closed_question_answer, question: closed_question, form: another_form)
      end
    ...
    context 'when violating unique radio question and option' do
      let(:radio_question) { create(:radio_question, :with_options) }
      let(:closed_question_answer) do
        create(:closed_question_answer, question: radio_question,
    ...
    context 'when violating unique checkbox question and option' do
      let(:checkbox_question) { create(:checkbox_question, :with_options) }
      let(:closed_question_answer) do
  ...
  describe '#destroy' do
    describe 'when form has expired' do
      subject(:closed_question_answer) do
        build(:closed_question_answer, form: create(:expired_form))
      end
