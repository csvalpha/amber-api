...
RSpec.describe Form::Form, type: :model do
  subject(:form) { FactoryBot.build_stubbed(:form) }

  describe '#valid' do
    it { expect(form).to be_valid }

    context 'when without a publish from date' do
      subject(:form) { FactoryBot.build_stubbed(:form, respond_from: nil) }

      it { expect(form).not_to be_valid }
    end

    context 'when without a publish until date' do
      subject(:form) { FactoryBot.build_stubbed(:form, respond_until: nil) }
    ...
    context 'when publish until is before publish at date' do
      subject(:form) do
        FactoryBot.build_stubbed(:form, respond_from: Date.tomorrow,
