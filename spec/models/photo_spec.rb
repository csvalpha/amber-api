...
RSpec.describe Photo do
  ...
  describe '#visibilty' do
    ...
    it { expect(described_class.joins(:photo_album).where(photo_albums: { visibility: %w[alumni public] }).count).to be 2 }
    it { expect(described_class.count - described_class.joins(:photo_album).where(photo_albums: { visibility: %w[alumni public] }).count).to be 1 }
  end
