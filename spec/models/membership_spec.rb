...
RSpec.describe Membership, type: :model do
  ...
  describe '#valid?' do
    ...
    context 'when without a group' do
      subject(:membership) { build_stubbed(:membership, group: nil) }

      it { expect(membership).not_to be_valid }
    end

    context 'when without a user' do
      subject(:membership) { build_stubbed(:membership, user: nil) }

      it { expect(membership).not_to be_valid }
    end

    context 'when without a start_date' do
      subject(:membership) { build_stubbed(:membership, start_date: nil) }
    ...
    context 'unique_on_time_interval?' do
      let(:existing_membership) do
        create(:membership, start_date: 2.weeks.ago,
                            end_date: 1.week.ago)
      end

      context 'with start_time within existing range' do
        subject(:membership) do
          build(:membership, user: existing_membership.user,
