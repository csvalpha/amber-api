...
RSpec.describe Forum::Thread, type: :model do
  subject(:thread) { FactoryBot.build_stubbed(:thread) }

  describe '#valid' do
    it { expect(thread.valid?).to be true }

    context 'when without a title' do
      subject(:thread) { FactoryBot.build_stubbed(:thread, title: nil) }

      it { expect(thread.valid?).to be false }
    end

    context 'when without an author' do
      subject(:thread) { FactoryBot.build_stubbed(:thread, author: nil) }

      it { expect(thread.valid?).to be false }
    end

    context 'when without a category' do
      subject(:thread) { FactoryBot.build_stubbed(:thread, category: nil) }
  ...
  describe 'it updates the category after save' do
    subject(:thread) { FactoryBot.build(:thread) }
