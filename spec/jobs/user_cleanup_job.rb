...
RSpec.describe UserCleanupJob, type: :job do
  describe '#perform' do
    ...
    let(:almost_remove_user) { FactoryBot.create(:user, memberships: [FactoryBot.create(:membership, start_date: 19.months.ago, end_date: 18.months.ago)]) }
    let(:remove_user) { FactoryBot.create(:user, memberships: [FactoryBot.create(:membership, start_date: 22.months.ago, end_date: 21.months.ago)]) }
