require 'rails_helper'

RSpec.describe ArticlePolicy, type: :policy do
  subject(:policy) { described_class }

  let(:user) { build(:user) }

  permissions :update? do
    describe 'when article is not owned' do
      it { expect(policy).not_to permit(user, build_stubbed(:article)) }
    end

    describe 'when article is owned' do
      it { expect(policy).to permit(user, build_stubbed(:article, author: user)) }
    end

    describe 'when with permission' do
      let(:record_permission) { 'article.update' }
      let(:user) { create(:user, user_permission_list: [record_permission]) }

      it { expect(policy).to permit(user, build_stubbed(:article)) }
    end
  end
end
