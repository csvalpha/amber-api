...
RSpec.describe Form::ClosedQuestionOptionPolicy, type: :policy do
  ...
  let(:form) { FactoryBot.build(:form, author: user) }

  permissions :update?, :destroy? do
    describe 'when closed question option is not owned' do
      it { expect(policy).not_to permit(user, FactoryBot.build_stubbed(:closed_question_option)) }
    end

    describe 'when closed question option is owned' do
      it { expect(policy).to permit(user, FactoryBot.build(:closed_question_option, form: form)) }
    end
  end
